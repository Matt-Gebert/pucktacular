...
uint8 SW_Comparator_initVar = 0u;

/* Internal functions definitoin */
static void SW_Comparator_trimAdjust(uint8 nibble) ;

/* static SW_Comparator_backupStruct  SW_Comparator_backup; */
#if (CY_PSOC5A)
    static SW_Comparator_LOWPOWER_BACKUP_STRUCT  SW_Comparator_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 SW_Comparator_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: SW_Comparator_Init
********************************************************************************
*
* Summary:
...
void SW_Comparator_Init(void) 
{
    ...
    SW_Comparator_SetSpeed(SW_Comparator_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( SW_Comparator_DEFAULT_HYSTERESIS == 0u )
        SW_Comparator_CR |= SW_Comparator_HYST_OFF;
    #else
        SW_Comparator_CR &= (uint8)(~SW_Comparator_HYST_OFF);
    #endif /* SW_Comparator_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( SW_Comparator_DEFAULT_PWRDWN_OVRD == 0u )
            SW_Comparator_CR &= (uint8)(~SW_Comparator_PWRDWN_OVRD);
        #else 
            SW_Comparator_CR |= SW_Comparator_PWRDWN_OVRD;
        #endif /* SW_Comparator_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    SW_Comparator_CR |= SW_Comparator_MX_AO;

    /* Set default sync */
    SW_Comparator_CLK &= (uint8)(~SW_Comparator_SYNCCLK_MASK);
    #if ( SW_Comparator_DEFAULT_BYPASS_SYNC == 0u )
        SW_Comparator_CLK |= SW_Comparator_SYNC_CLK_EN;
    #else
        SW_Comparator_CLK |= SW_Comparator_BYPASS_SYNC;
    #endif /* SW_Comparator_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: SW_Comparator_Enable
********************************************************************************
*
* Summary:
...
{
    SW_Comparator_PWRMGR |= SW_Comparator_ACT_PWR_EN;
    SW_Comparator_STBY_PWRMGR |= SW_Comparator_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(SW_Comparator_restoreReg == 1u)
        {
            SW_Comparator_CR = SW_Comparator_lowPowerBackup.compCRReg;

            /* Clear the flag */
            SW_Comparator_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
/*******************************************************************************
* Function Name: SW_Comparator_Start
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  SW_Comparator_initVar: Is modified when this function is called for the 
...
void SW_Comparator_Start(void) 
{
    ...
    if ( SW_Comparator_initVar == 0u )
    {
        SW_Comparator_Init();
        
        SW_Comparator_initVar = 1u;
    }   
...
/*******************************************************************************
* Function Name: SW_Comparator_Stop
********************************************************************************
*
* Summary:
...
void SW_Comparator_Stop(void) 
{
    ...
    SW_Comparator_PWRMGR &= (uint8)(~SW_Comparator_ACT_PWR_EN);
    SW_Comparator_STBY_PWRMGR &= (uint8)(~SW_Comparator_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        SW_Comparator_restoreReg = 1u;

        /* Save the control register before clearing it */
        SW_Comparator_lowPowerBackup.compCRReg = SW_Comparator_CR;
        SW_Comparator_CR = SW_Comparator_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: SW_Comparator_SetSpeed
********************************************************************************
*
* Summary:
...
void SW_Comparator_SetSpeed(uint8 speed) 
{
    ...
    SW_Comparator_CR = (SW_Comparator_CR & (uint8)(~SW_Comparator_PWR_MODE_MASK)) |
                           (speed & SW_Comparator_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == SW_Comparator_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            SW_Comparator_TR = SW_Comparator_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            SW_Comparator_TR0 = SW_Comparator_HS_TRIM_TR0;
            SW_Comparator_TR1 = SW_Comparator_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            SW_Comparator_TR = SW_Comparator_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            SW_Comparator_TR0 = SW_Comparator_LS_TRIM_TR0;
            SW_Comparator_TR1 = SW_Comparator_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: SW_Comparator_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( SW_Comparator_WRK & SW_Comparator_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: SW_Comparator_trimAdjust
********************************************************************************
*
* Summary:
...
static void SW_Comparator_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = SW_Comparator_WRK & SW_Comparator_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                SW_Comparator_TR |= SW_Comparator_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                SW_Comparator_TR0 |= SW_Comparator_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                SW_Comparator_TR |= SW_Comparator_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                SW_Comparator_TR1 |= SW_Comparator_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        else
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                SW_Comparator_TR += 0x10u;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                SW_Comparator_TR1 += 1u;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
        ...
        if ((SW_Comparator_WRK & SW_Comparator_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: SW_Comparator_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 SW_Comparator_ZeroCal(void) 
{
    ...
    tmpSW0 = SW_Comparator_SW0;
    tmpSW2 = SW_Comparator_SW2;
    tmpSW3 = SW_Comparator_SW3;

     /* Clear routing for inP, retain routing for inN */
    SW_Comparator_SW0 = 0x00u;
    SW_Comparator_SW2 = 0x00u;
    SW_Comparator_SW3 = tmpSW3 & (uint8)(~SW_Comparator_CMP_SW3_INPCTL_MASK);

    /* Preserve original configuration
     * - turn off Hysteresis
     * - set calibration bit - shorts inN to inP
    */
    tmpCR = SW_Comparator_CR;
    SW_Comparator_CR |= (SW_Comparator_CAL_ON | SW_Comparator_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        SW_Comparator_TR = SW_Comparator_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        SW_Comparator_TR0 = SW_Comparator_DEFAULT_CMP_TRIM;
        SW_Comparator_TR1 = SW_Comparator_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (SW_Comparator_CR & SW_Comparator_PWR_MODE_MASK) == SW_Comparator_PWR_MODE_FAST)
    {
        SW_Comparator_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        SW_Comparator_trimAdjust(1u);
		SW_Comparator_trimAdjust(0u);
    }
   
    /* Restore Config Register */
    SW_Comparator_CR = tmpCR;
    
    /* Restore routing registers for inP */
    SW_Comparator_SW0 = tmpSW0;
    SW_Comparator_SW2 = tmpSW2;
    SW_Comparator_SW3 = tmpSW3;
    
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) SW_Comparator_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)SW_Comparator_TR1 << 8u) | (SW_Comparator_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: SW_Comparator_LoadTrim
********************************************************************************
*
* Summary:
...
void SW_Comparator_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: SW_Comparator_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: SW_Comparator_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
